...
const ProductView = ({ params }: Props) => {
	...
	const deleteProduct = useCallback(async () => {
		try {
			const { data } = await staffApi.delete(`/product/${product?._id}`)
			if (data.success) {
				router.back()
	...
	const toggleListing = useCallback(async () => {
		try {
			const { data } = await staffApi.put(`/product/list/${product._id}`, { listed: !product.listed })
	...
	return (
		<div className="flex gap-5 flex-col w-full h-full">
			...
			<div className="flex items-center gap-5 h-full w-full">
				...
				<div className="flex flex-col w-80 p-5 bg-white drop-shadow-lg rounded-lg h-full">
					...
					</div>
					{
						staff.manager && <div className="flex gap-2 items-center ">
							...
							<EditProduct api={staffApi} product={product} setProduct={setProduct}>
				...
				<ProductGraph id={params.id} api={staffApi} />
			</div>
		</div>
	)
}
